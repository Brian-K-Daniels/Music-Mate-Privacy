...
public static class SessionTableColumnDefinitions
{
    ...
    [
        ...
        new("CorrectPercent", "Pc%",
            "Pitch percent correct: 100 × (notes eventually played correctly) ÷ (correct count + total wrong attempts). Wrong attempts are summed from per-note retry feedback."),
        new("Tmg", "Tmg",
        ...
        new("Ovrl", "Ovrl",
            "Overall accuracy percentage. When timing accuracy is available, the average of pitch accuracy (Pc%) and timing accuracy (Tmg); otherwise equals pitch accuracy alone."),
        new("PitchRight", "P+",
            "Count of non-rest note attempts where pitch was correct."),
        new("PitchWrong", "P−",
            "Count of non-rest note attempts where pitch was wrong."),
        new("TimingRight", "T+",
